...
from utils.abi import zenithswap_abi, zenith_liquidity
from data.const import rpc, router_address, WPHRS_address, stables_data, liq_address
...
async def swap_from_native(wallet: Account, token, w3: AsyncWeb3):
    ...
    try:
        ...
        logger.info(wallet.address, f'{Fore.YELLOW}[Zenith]{Style.RESET_ALL} Trying to swap {amount} PHRS to {token['name']}')
        ...
        if tx_receipt.status == 1:
            logger.success(wallet.address, f'{Fore.YELLOW}[Zenith]{Style.RESET_ALL} Successfully swapped {amount} PHRS to {token['name']}')
            return True
        else:
            logger.error(wallet.address, f'{Fore.YELLOW}[Zenith]{Style.RESET_ALL} Swap error: {tx_receipt}')

    except Exception as e:
        logger.error(wallet.address, f'{Fore.YELLOW}[Zenith]{Style.RESET_ALL} An error occurred while swapping from native: {e}')
...
async def swap_from_stable(wallet: Account, token1, token2, w3: AsyncWeb3):
    ...
    try:
        ...
        logger.info(wallet.address, f'{Fore.YELLOW}[Zenith]{Style.RESET_ALL} Trying to swap {amount} {token1['name']} to {token2['name']}')
        ...
        tx_receipt = await w3.eth.wait_for_transaction_receipt(tx_hash)
        if tx_receipt.status == 1:
            logger.success(wallet.address, f'{Fore.YELLOW}[Zenith]{Style.RESET_ALL} Successfully swapped {amount} {token1['name']} to {token2['name']}')
            return True
        else:
            logger.error(wallet.address, f'{Fore.YELLOW}[Zenith]{Style.RESET_ALL} Swap error: {tx_receipt}')

    except Exception as e:
        logger.error(wallet.address, f'{Fore.YELLOW}[Zenith]{Style.RESET_ALL} An error occurred while swapping from stable: {e}')
...
async def zenith_handle_swap(wallet, w3=AsyncWeb3(AsyncWeb3.AsyncHTTPProvider(rpc))):
    ...
    try:
        ...
        logger.error(wallet.address, f'{Fore.YELLOW}[Zenith]{Style.RESET_ALL} Error while handling random swap: {e}')
